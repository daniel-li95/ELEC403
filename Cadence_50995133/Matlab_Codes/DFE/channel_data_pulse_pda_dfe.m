...
clear pulse_xaxis;
[max_data_ch,max_data_ch_idx]=max(abs(data_channel_dfe));
pulse_xaxis(1,:)=((1:size(data_channel_dfe,2))-max_data_ch_idx)./bit_period;

for i=1:101
sample_points(i)=max_data_ch_idx+sample_offset+(i-11)*bit_period;
end
sample_values=data_channel_dfe(sample_points);
sample_points=(sample_points-max_data_ch_idx)./bit_period;


figure;
%J=plot(pulse_noeq_xaxis,2*data_channel_noeq,'-r'); hold on;
H=plot(pulse_xaxis,data_channel_dfe,'-r'); hold on;
